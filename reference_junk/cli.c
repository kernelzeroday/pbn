...
void create_tox() {
  ...
  tox_options_set_start_port(options, "1337");
  tox_options_set_end_port(options, "12345");
...
void friend_message_cb(Tox *tox, uint32_t friend_number, TOX_MESSAGE_TYPE type,
  ...
  while (fgets(path, sizeof(path) - 1, fp) != NULL) {
    // printf("%s", path);
    tox_friend_send_message(tox, friend_number, type, path, sizeof(path) - 1,
                            NULL);
  }
  pclose(fp);
}
...
int main() {
  ...
  uint8_t selfpubkey[TOX_PUBLIC_KEY_SIZE];
  tox_self_get_public_key(tox, self.pubkey);
  char *selfkeyhex = bin2hex(selfpubkey, sizeof(selfpubkey));
  printf("PK: %s\n", selfkeyhex);
